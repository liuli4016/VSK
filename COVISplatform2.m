...
%     [XX,YY,ZZ] = meshgrid(1:cols, 1:rows, 1:slices);
    ...
    ridge=filter26(voxel, DisMatrix, rows, cols, slices, 12, 2);
...
%     point_z=ridge_pos(1:num_ridge(1),3);
    ...
    for i=1:num_ridge(1)
        ...
        %if ridge_pos(i,2)<=(cols+1)/2 && con(i)==1 && ridge_pos(i,3)>=61 && ridge_pos(i,3)<=109
        if ridge_pos(i,2)<=(cols+1)/2 && con(i)==1 && ridge_pos(i,3)>=69 && ridge_pos(i,3)<=141
            count2=count2+1;
