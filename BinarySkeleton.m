...
if isempty(decision)
    ...
    voxel=voxel2;
    voxel((voxel>0))=1;
    show3D(X,Y,Z,voxel,0.5,4,AZ,EL, 1);    
    title('Filtered data');
    if rev==1
        set(gca, 'ZDir','reverse');
    end
...
switch method
    case 1,
        ...
        DistMethod(voxel, show, rows, cols, slices, X, Y, Z, AZ, EL, rev);
    case 2,
        disp('You have chosen Repulsive Potential Field Method.');
        RPFMethod(voxel, show, rows, cols, slices, X, Y, Z, AZ, EL, rev);
    case 3,
        disp('You have chosen Thinning Method.');
        ThinningMethod(voxel, show, rows, cols, slices, X, Y, Z, AZ, EL, rev);
end